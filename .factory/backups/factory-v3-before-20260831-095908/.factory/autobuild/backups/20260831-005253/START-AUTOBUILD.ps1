function Is-Protected($relative) {
    if ([string]::IsNullOrWhiteSpace($relative)) { return $false }
    $p = $relative.Replace("/", "\").TrimStart("\")
    if ($p -like ".factory\*" -or $p -like "node_modules\*" -or $p -like "dist\*" -or $p -like "build\*" -or $p -like ".next\*" -or $p -like "coverage\*") { return $true }
    return $false
}

function Is-Active($relative) {
    ...
    # Factory/internal files are not active project source.
    if ($normalized -like ".factory\*") {
        return $false
    }

    # Dependencies are not active source.
    if ($normalized -like "node_modules\*") {
        return $false
    }

    # Common generated/build output is not active source.
    if ($normalized -like "dist\*" -or
        $normalized -like "build\*" -or
        $normalized -like ".next\*" -or
        $normalized -like "coverage\*") {
        return $false
    }
...
function Is-Excluded($relative) {

    $p = $relative.Replace("/", "\").TrimStart("\")

    # NEVER back up the factory's own backup tree.
    if ($p -eq ".factory") { return $true }
    if ($p.StartsWith(".factory\")) { return $true }

    # Dependencies/build output are never part of transactional source backup.
    if ($p -eq "node_modules") { return $true }
    if ($p.StartsWith("node_modules\")) { return $true }

    if ($p -eq "dist") { return $true }
    if ($p.StartsWith("dist\")) { return $true }

    if ($p -eq ".git") { return $true }
